...
import com.yalin.googleio2016.io.model.Session;
...
import com.yalin.googleio2016.provider.ScheduleDatabase;
...
public class SessionsHandler extends JSONHandler {
    ...
    public void makeContentProviderOperation(ArrayList<ContentProviderOperation> list) {
        ...
        if (incrementalUpdate) {
            LogUtil.d(TAG, "Doing incremental update for sessions.");
        } else {
            LogUtil.d(TAG, "Doing full (non-incremental) update for sessions.");
        ...
        LogUtil.d(TAG, "Sessions: " + (incrementalUpdate ? "INCREMENTAL" : "FULL") + " update. " +
    ...
    private HashMap<String, String> loadSessionHashCodes() {
        Uri uri = ScheduleContractHelper.setUriAsCalledFromSyncAdapter(
                ScheduleContract.Sessions.CONTENT_URI);
        LogUtil.d(TAG, "Loading session hashcodes for session import optimization.");
        Cursor cursor = null;
        try {
            cursor = mContext.getContentResolver().query(uri, SessionHashcodeQuery.PROJECTION,
                    null, null, null);
            if (cursor == null || cursor.getCount() < 1) {
                LogUtil.w(TAG, "Warning: failed to load session hashcodes. Not optimizing session import.");
                return null;
            }
            ...
            LogUtil.d(TAG, "Session hashcodes loaded for " + hashcodeMap.size() + " sessions.");
            return hashcodeMap;
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    private void buildSession(boolean isInsert, Session session,
        ...
        if (mSpeakerMap != null) {
            ...
            if (session.speakers != null) {
                for (int i = 0; i < session.speakers.length; ++i) {
                    if (mSpeakerMap.containsKey(session.speakers[i])) {
                        ...
                        LogUtil.w(TAG, "Unknown speaker ID " + session.speakers[i] + " in session " +
                                session.id);
                    }
                }
            }
            ...
            LogUtil.e(TAG, "Can't build speaker names -- speaker map is null");
        }
        ...
        try {
            ...
            LogUtil.d(TAG, "Ignoring invalid formatted session color: " + session.color);
        }
