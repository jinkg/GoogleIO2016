...
public class ScheduleHelper {
    ...
    public ArrayList<ScheduleItem> getScheduleData(long start, long end) {
        ...
        ArrayList<ScheduleItem> result =
                ScheduleItemHelper.processItems(mutableItems, immutableItems);
        if (BuildConfig.DEBUG || LogUtil.isLoggable()) {
            ScheduleItem previous = null;
            for (ScheduleItem item : result) {
                if ((item.flags & ScheduleItem.FLAG_CONFLICTS_WITH_PREVIOUS) != 0) {
                    LogUtil.d(TAG, "Schedule Item conflicts with previous. item=" + item + " previous=" + previous);
                }
                previous = item;
            }
        }
    ...
    protected void setSessionCounters(ArrayList<ScheduleItem> items, long dayStart, long dayEnd) {
        ...
        String liveStreamedOnlySelection = UIUtils.shouldShowLiveSessionsOnly(mContext)
                ? "AND IFNULL(" + Sessions.SESSION_LIVESTREAM_ID + ",'')!=''"
                : "";
        ...
        while (it.hasNext()) {
            ...
            if (i.type == ScheduleItem.FREE) {
                if (i.endTime < now) {
                    LogUtil.d(TAG, "Removing empty block in the past.");
                    it.remove();
                } else if (i.numOfSessions == 0) {
                    LogUtil.d(TAG, "Removing block with zero sessions: " + new Date(i.startTime) + "-" + new Date(i.endTime));
                    it.remove();
                } else {
    ...
    protected void addBlocks(long start, long end,
                             ArrayList<ScheduleItem> mutableItems,
