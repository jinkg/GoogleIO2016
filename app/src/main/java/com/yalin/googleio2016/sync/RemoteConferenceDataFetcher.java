...
import com.yalin.googleio2016.io.model.DataManifest;
import com.yalin.googleio2016.util.HashUtils;
import com.yalin.googleio2016.util.IOUtils;
...
public class RemoteConferenceDataFetcher {
    private static final String TAG = "RemoteDataFetcher";
    ...
    public String[] fetchConferenceDataIfNewer(String refTimestamp) throws IOException {
        if (TextUtils.isEmpty(mManifestUrl)) {
            LogUtil.w(TAG, "Manifest URL is empty (remote sync disabled!).");
            return null;
        }
        ...
        if (!TextUtils.isEmpty(refTimestamp)) {
            if (TimeUtils.isValidFormatForIfModifiedSinceHeader(refTimestamp)) {
                ...
                LogUtil.w(TAG, "Could not set If-Modified-Since HTTP header. Potentially downloading " +
        ...
        if (response == null) {
            LogUtil.e(TAG, "Request for manifest returned null response.");
            throw new IOException("Request for data manifest returned null response.");
        }

        int status = response.getStatus();
        if (status == HttpURLConnection.HTTP_OK) {
            LogUtil.d(TAG, "Server returned HTTP_OK, so new data is available.");
            mServerTimestamp = getLastModified(response);
            LogUtil.d(TAG, "Server timestamp for new data is: " + mServerTimestamp);
            String body = response.getBodyAsString();
            if (TextUtils.isEmpty(body)) {
                LogUtil.e(TAG, "Request for manifest returned empty data.");
                throw new IOException("Error fetching conference data manifest: no data.");
            }
            LogUtil.d(TAG, "Manifest " + mManifestUrl + " read, contents: " + body);
            mBytesDownloaded += body.getBytes().length;
            return processManifest(body);
        } else if (status == HttpURLConnection.HTTP_NOT_MODIFIED) {
            // data on the server is not newer than our data
            LogUtil.d(TAG, "HTTP_NOT_MODIFIED: data has not changed since " + refTimestamp);
            return null;
        } else {
            LogUtil.e(TAG, "Error fetching conference data: HTTP status " + status + " and manifest " +
                    mManifestUrl);
    ...
    private String getManifestUrl() {
        ...
        if (urlOverrideFile.exists()) {
            try {
                ...
                LogUtil.d(TAG, "Debug URL override active: " + overrideUrl);
                return overrideUrl;
    ...
    private String fetchFile(String url) throws IOException {
        ...
        if (!url.contains("://")) {
            if (TextUtils.isEmpty(mManifestUrl) || !mManifestUrl.contains("/")) {
                LogUtil.e(TAG, "Could not build relative URL based on manifest URL.");
                return null;
            }
        ...
        LogUtil.d(TAG, "Attempting to fetch: " + sanitizeUrl(url));
        ...
        try {
            ...
            ex.printStackTrace();
            LogUtil.e(TAG, "IOException getting file from cache.");
        ...
        LogUtil.d(TAG, "Cache miss. Downloading from network: " + sanitizeUrl(url));
        ...
        LogUtil.d(TAG, "HTTP response " + response.getStatus());
        if (response.getStatus() == HttpURLConnection.HTTP_OK) {
            ...
            LogUtil.d(TAG, "Successfully downloaded from network: " + sanitizeUrl(url));
            mBytesDownloaded += body.getBytes().length;
            writeToCache(url, body);
            mCacheFilesToKeep.add(getCacheKey(url));
            return body;
        } else {
            LogUtil.e(TAG, "Failed to fetch from network: " + sanitizeUrl(url));
    ...
    private String[] processManifest(String manifestJson) throws IOException {
        LogUtil.d(TAG, "Processing data manifest,length " + manifestJson.length());
        ...
        if (manifest.format == null || !manifest.format.equals(MANIFEST_FORMAT)) {
            LogUtil.e(TAG, "Manifest has invalid format spec: " + manifest.format);
        ...
        if (manifest.data_files == null || manifest.data_files.length == 0) {
            LogUtil.e(TAG, "Manifest does not list any files. Nothing done.");
            return null;
        }

        LogUtil.d(TAG, "Manifest lists " + manifest.data_files.length + " data files.");
        ...
        for (int i = 0; i < manifest.data_files.length; i++) {
            ...
            LogUtil.d(TAG, "Processing data file: " + sanitizeUrl(url));
            jsons[i] = fetchFile(url);
            if (TextUtils.isEmpty(jsons[i])) {
                LogUtil.e(TAG, "Failed to fetch data file: " + sanitizeUrl(url));
                throw new IOException("Failed to fetch data file " + sanitizeUrl(url));
            }
        }
        LogUtil.d(TAG, "Got " + jsons.length + " data files.");
        cleanUpCache();
        return jsons;
    }
    ...
    private String loadFromCache(String url) throws IOException {
        ...
        if (cacheFile.exists()) {
            LogUtil.d(TAG, "Cache hit " + cacheKey + " for " + sanitizeUrl(url));
            return IOUtils.readFileAsString(cacheFile);
        } else {
            LogUtil.d(TAG, "Cache miss " + cacheKey + " for " + sanitizeUrl(url));
            return null;
        }
    }
    ...
    private void writeToCache(String url, String body) throws IOException {
        ...
        LogUtil.d(TAG, "Wrote to cache " + cacheKey + " --> " + sanitizeUrl(url));
    }

    // Delete unnecessary files from our cache
    private void cleanUpCache() {
        LogUtil.d(TAG, "Starting cache cleanup, " + mCacheFilesToKeep.size() + " URLs to keep.");
        File dir = new File(mContext.getCacheDir() + File.separator + CACHE_DIR);
        if (!dir.exists()) {
            LogUtil.d(TAG, "Cleanup complete (there is no cache).");
            return;
        }
        ...
        for (File file : dir.listFiles()) {
            if (mCacheFilesToKeep.contains(file.getName())) {
                LogUtil.d(TAG, "Cache cleanup: KEEEPING " + file.getName());
                ++kept;
            } else {
                LogUtil.d(TAG, "Cache cleanup: DELETING " + file.getName());
                file.delete();
                ++deleted;
            }
        }
        LogUtil.d(TAG, "End of cache cleanup. " + kept + " files kept, " + deleted + " deleted.");
    }
